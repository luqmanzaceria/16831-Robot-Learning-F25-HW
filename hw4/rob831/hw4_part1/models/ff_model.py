...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # recover the unnormalized delta
        delta_pred = delta_pred_normalized * (delta_std + 1e-8) + delta_mean
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = (delta - data_statistics['delta_mean']) / (data_statistics['delta_std'] + 1e-8)

        # Convert everything to torch tensors
        obs_torch = ptu.from_numpy(observations)
        acs_torch = ptu.from_numpy(actions)
        target_torch = ptu.from_numpy(target)
        ...
        _, delta_pred_normalized = self.forward(
            obs_torch, acs_torch,
            obs_mean, obs_std, acs_mean, acs_std,
            delta_mean, delta_std
        )
        loss = self.loss(delta_pred_normalized, target_torch)
