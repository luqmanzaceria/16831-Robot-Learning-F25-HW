...
class PGAgent(BaseAgent):
    def __init__(self, env, agent_params):
        ...
        self.num_policy_gradient_steps_per_batch = self.agent_params.get('num_policy_gradient_steps_per_batch', 1)
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        # Multi-step policy gradient: take multiple gradient steps on the same batch
        train_logs = []
        for _ in range(self.num_policy_gradient_steps_per_batch):
            train_log = self.actor.update(observations, actions, advantages, q_values)
            train_logs.append(train_log)

        # Return the last train log (most recent loss values)
        return train_logs[-1]
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            # unnormalize the predicted values to match the mean and std of current q_values
            # so that they can be subtracted from q_values to compute the advantage
            values = values_normalized * (q_values.std() + 1e-8) + q_values.mean()
    ...
    def _discounted_return(self, rewards):
        ...
        # compute the total discounted return from the entire trajectory
        total_return = np.sum(rewards * self.gamma ** np.arange(len(rewards)))
        # return the same value for all timesteps
        discounted_returns = np.full_like(rewards, total_return, dtype=float)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounted_cumsums = np.zeros_like(rewards, dtype=float)
        T = len(rewards)
        for t in range(T):
            discounted_cumsums[t] = np.sum([rewards[t_p] * self.gamma ** (t_p - t) for t_p in range(t, T)])
