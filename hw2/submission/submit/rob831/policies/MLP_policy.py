...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        # compute the policy loss using the policy gradient objective
        ...
        if self.discrete:
            log_probs = action_dist.log_prob(actions.squeeze(-1) if actions.ndim > 1 else actions)
        else:
            log_probs = action_dist.log_prob(actions)
        # compute the loss (negative of the objective, since optimizer minimizes)
        ...
        if self.nn_baseline:
            ...
            # normalize q_values to mean 0, std 1
